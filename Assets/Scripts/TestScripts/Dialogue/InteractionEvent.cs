    ...
    public void SetNextContext()
    {
        ...
        if (num < dialogue.dialouses.Length)
        {
            ...
            CallCommand(ref precommands);//이전에 실행되어야할 명령어들
                                         //이게 두번 일어나는듯?
            if (_Uimanager.none_state==true&& Regex.Replace(dialogue.dialouses[num].dir, @"[^a-zA-Z]", "")!="none")
            {
                Debug.Log("load image");
                //_Uimanager.LoadImage();
                _Uimanager.SetImage(dialogue.dialouses[num+1].image_name, dialogue.dialouses[num+1].dir);
            }
